...
struct SavePlacement{ //pas un pointeur come on prend une copie parce que si c'est des pointeur on peu pas le comparer a ceux dans availShapes
    ...
    int indiceDansTab=-1;
    char id='O';
    int nbOpti=0;//on verifiera ca quand in decrement pour etre sur de ne pas reprendre le meme 2 fois d'affiler et eviter de refaire idefiniment la meme operaton
    int nbCheck=0;
};
...
private:
    ...
    //donne aussi en qq sorte la prochiane shape a placer
public:
